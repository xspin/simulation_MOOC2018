...
function f = virtual_force(obj, type, i, j, k)
    if nargin<5
        k = i;
    end
    a=0.9;
    ...
    eij = pij/dij;
    vij = [sin(yaw), -cos(yaw)];
    s = sign(dot(obj.nodes(j).p-obj.nodes(k).p, vij));
    if k==i 
        s = -s;
    else
        vij = s*vij;
...
function obj = update_state_CBMM(obj)
    ...
    for i=2:obj.n_nodes
        ...
        % loose force
        for j=obj.nodes(i).neighbor
            f = f + virtual_force(obj, 'cov', i, j)*50;
        end
    ...
    for i=2:obj.n_nodes
        ...
        % loose force
        for j=obj.nodes(i).neighbor
            f = f + virtual_force(obj, 'loo', i, j);
        end
        if obj.nodes(i).type == 'w'
            % mobility for ONs
            for j=obj.nodes(i).neighbor
                f = f + virtual_force(obj, 'con', i, j);
            end
        elseif obj.nodes(i).type == 'g'
            % mobility for CMs
            j=obj.nodes(i).pid;
            f = f + virtual_force(obj, 'con', i, j) + virtual_force(obj, 'cov', i, j);
        elseif obj.nodes(i).type == 'b'
            % mobility for CHs
            cpid = obj.nodes(i).cpid;
            if cpid>0
                for k=[i, obj.nodes(i).cluster_member]
                    nb = obj.nodes(k).neighbor;
                    ch = [obj.nodes(nb).chid];
                    nb = nb(ch ~= cpid);
                    for j=nb
                        %f = f + virtual_force(obj, 'con', k, j) + virtual_force(obj, 'cov', k, j); 
                    end
                end
                %obj.nodes(i).v = obj.nodes(cpid).v+0.1;
            end
        end        
        % change state acoording virtual force
        obj.nodes(i).yaw = vang(f);
%         obj.nodes(i).v = obj.velocity + 0.1*norm(force{1})/obj.range_transmission;
    end
    %return
...
function obj = clustering_MPCA(obj)
    ...
    for i=1:length(obj.chs)
        ch_id=obj.chs(i);
        if length(obj.nodes(ch_id).cluster_member)<min_cm...
                || (max([obj.nodes(obj.nodes(ch_id).cluster_member).weight_mpca])>obj.nodes(ch_id).weight_mpca && rand<0.01)
            for j=obj.nodes(ch_id).cluster_member
                obj.nodes(j) = obj.nodes(j).bewhite();
            end
            obj.nodes(ch_id) = obj.nodes(ch_id).bewhite();
...
function obj = clustering_HDCA(obj)
    for i=1:obj.n_nodes
        if obj.nodes(i).type == 'w'
            ...
            k = 0;
            for j=wnb
               if length(obj.nodes(j).neighbor)>maxdegree
                   maxdegree = length(obj.nodes(j).neighbor);
                   k = j;
               end
            end
            %if length(obj.nodes(i).neighbor) > maxdegree ||(i<=k && length(obj.nodes(i).neighbor)==maxdegree)
            if i==k && length(wnb)>1
                obj.chs(end+1) = i;
                myprintf(' Add CH: %d\n', i);
                obj.nodes(i).type = 'b';
...
%         wnb = [wnb, 
        for j=obj.nodes(ch_id).cluster_member
            if length(obj.nodes(j).neighbor)>length(obj.nodes(ch_id).neighbor)
                flag = true;
            end
        end
        if length(obj.nodes(ch_id).cluster_member)<min_cm || (flag && rand<0.03)
            for j=obj.nodes(ch_id).cluster_member
                obj.nodes(j) = obj.nodes(j).bewhite();
            end
            obj.nodes(ch_id) = obj.nodes(ch_id).bewhite();
